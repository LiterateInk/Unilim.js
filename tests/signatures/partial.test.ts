...
describe("iut/signatures [partial]", () => {
  ...
  it("should parse the 5th semester's 1st unit", () => {
...

    {
      ...
      expect(module.average).toBe(13);
      expect(module.exams).toBeArrayOfSize(1);

      const exam = module.exams[0];
      expect(exam.date).toEqual(new Date("2025-11-20"));
      expect(exam.average).toBe(13);
      expect(exam.globalAverage).toBe(12.56);
      expect(exam.coefficient).toBe(1.0);
      expect(exam.name).toBe("Moyenne");
    }
